...
That is not an assertion — `python3 tools/kicad-verify.py gerber` checks it against a baseline frozen from
...
```bash
python3 tools/kicad-verify.py gerber    # copper unchanged vs the frozen v1.4 baseline
python3 tools/kicad-verify.py netlist            # no pin moved to a different net
python3 tools/kicad-verify.py rules            # ERC and DRC counts by type
python3 tools/render.py mycheck  # a render into verify/renders/ (gitignored)
python3 tools/kicad-verify.py models   # 3D coverage, resolving models only
```
